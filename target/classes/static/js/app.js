...
function handleApplyFilters() {
    ...
    if (category) {
        ...
        console.log(`Filtered by category: ${category}, ${filtered.length} products found`);
    }

    // Filter by price if entered
    if (maxPrice) {
        const price = parseFloat(maxPrice);
        if (!isNaN(price)) {
            filtered = filtered.filter(product => product.price <= price);
            console.log(`Filtered by price <= ${price}, ${filtered.length} products found`);
        }
    }
    ...
    try {
        ...
        if (!response.ok) {
            if (response.status === 401) {
                console.warn('Unauthorized (401): User session expired or not logged in');
                ...
                throw new Error('Products endpoint not found (404). Please check the API configuration.');
        ...
        if (error instanceof TypeError) {
            console.error('Network error - unable to reach the backend API:', error.message);
            throw new Error('Network error: Cannot reach the backend server at ' + API_BASE_URL);
...
/**
 * Logs application initialization details to the console.
 */
